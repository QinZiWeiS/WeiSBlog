...
tags:
  ...
  - 面经
---
有关this指向的代码题解释。末尾有手写new，call，apply，bind，参考了众多文章总结出了比较简洁且容易理解的写法
...
var name = 'window'
var persion1 = new Person('QinZiWei')
...
其实大家不用担心，做这类题只要谨记法则，找到规律，什么题都可以解了！
...
var QinZiWei = create(Person, 'QinZiWei')
console.log(QinZiWei) // Person{ name: 'QinZiWei' }
QinZiWei.eat() // 'Eatting'
```
...
Function.prototype.MyBind = function (context) {
    ...
    return function () {
        ...
        // 返回绑定this后的函数，传入两次保存的参数
        //考虑返回函数有返回值做了return
        return self.apply(context, arg.concat(innerArg))
    }
}
```
...
## 总结关键点
- 普通函数，有显示绑定直接看显示绑定，没有就看是谁最后调用的它
- 箭头函数，什么都不管，只看是在哪个作用域下，对象没有作用域！！！函数有！对象身上的函数作用域也有！！